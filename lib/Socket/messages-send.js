...
const makeMessagesSocket = (config) => {
    ...
    const chisa = new Z(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await chisa.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = chisa.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await chisa.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await chisa.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await chisa.handleInteractive(content, jid, quoted);
                        ...
                        case 'INTERACTIVE_BUTTONS':
                            const ibContent = await chisa.handleInteractiveButtons(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await chisa.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await chisa.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await chisa.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await chisa.handleGroupStory(content, jid, quoted)
                    }
                }
