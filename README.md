...
<div align="center">
  <h1>Chisa BAILEYS</h1>
  <img src="https://files.catbox.moe/7m2cnz.jpg" alt="Thumbnail" width='100%' />
</div>
...
```javascript
await Chisa.setLabelGroup(jid, string)
```
---
### Delay
Sleep code ( hehe )
...
```javascript
await Chisa.delay(seconds) // ex. 3 ( 3 seconds )
// example
await Chisa.delay(3)
```
---
...
```javascript
await Chisa.newsletterMultipleFollow(jids)
// Example
await Chisa.newsletterMultipleFollow("120xxxxxxx@newsletter 120xxxxxxxxx@newsletter 120xxxxxxx@newsletter")
```
---
### Check banned number
...
```javascript
Chisa.checkBanned(jid)
```
---
### Edit Message
Edit your previously sent message
```js
await Chisa.edit(m, newText)
// Example
await Chisa.edit(m, "this is edited message")
```
Notes
...
```javascript
await Chisa.sendStatusMention(content, jid);
```
        ...
        nativeFlowMessage: {        
            messageParamsJson: JSON.stringify({          
                ...
                tap_target_configuration: {            
                    ...
                    canonical_url: "https://Chisa.site",            
...
```javascript
const resized = await Chisa.resize(buffer, 200, 200)
```
---
### Convert
...
```javascript
const mp4 = await Chisa.convert(buffer, { to: "mp4" })
const webp = await Chisa.convert(buffer, { to: "webp" })
const mp3 = await Chisa.convert(buffer, { to: "mp3" })
```
---
### To Sticker
...
```javascript
const sticker = await Chisa.toSticker(buffer)
const sticker = await Chisa.toSticker(buffer, { quality: 90 })
```
---
### Compress
...
```javascript
const compressed = await Chisa.compress(buffer, { quality: 50 })
```
---
### Metadata
...
```javascript
const info = await Chisa.metadata(buffer)
```
